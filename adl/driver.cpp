...
namespace adl {
  ...
  Driver::Driver() : scanner(*this), parser(scanner, *this), loc(0) {
    fillTypeTable();
                       ...
                       std::map<int, std::vector<std::string> > *_systmap)
  {
    ...
    std::cout << "\n\nPART: ";
    for(auto& l: *ListParts) std::cout << l.first << ", ";
    std::cout << "\n\nOBJ: ";
    for(auto& l: *ObjectCuts) std::cout << l.first << ", ";
    std::cout << "\n\nNODE: ";
    for(auto& l: *NodeVars) std::cout << l.first << ", ";
    std::cout << "\n";
