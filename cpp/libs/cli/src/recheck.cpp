...
std::string schema_of(const std::string& text) {
  ...
  // Fall back to a cheap scan so a /1 document can be named even if it
  // does not deserialize as /2.
    ...
    if (std::filesystem::is_directory(p, ec)) {
      ...
      if (found.empty()) {
        ...
        err = "no *.json certificate bundles in directory " + a + " (" +
              ...
              "Bundles are produced by `smash2_cpp verify --combine " +
              a + "`";
        return 2;
      }
...
int main(int argc, char** argv) {
  ...
  if (args.empty() || std::find(args.begin(), args.end(), "--help") != args.end() ||
    ...
    std::cerr << "usage: smash2_cpp-recheck BUNDLE.json... | DIR...\n";
    std::cerr << "re-checks smash2 `verify --combine` certificate bundles ("
  ...
  if (collect_inputs(args, files, err) != 0) {
    std::cerr << "smash2_cpp-recheck: " << err << "\n";
    return 2;
  }
