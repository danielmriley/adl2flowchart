...
`unsat_core` maps `:named` symbols onto current frames only; a stale or
unmapped name is `None` (not a truncated core). An empty `()` core stays
`Some([])` so certify-on can fail closed. `classify` is the single Bug-5
mapping: `(error …)` / `unsupported` / `unknown` / `timeout` are
`SatResult::Unknown`, never Unsat.
