#!/usr/bin/env bash
# P3 formula dump gate over cpp/tests/formula_gate_files.txt.
...
if [[ "$GATE_ORACLE" == "1" ]]; then
  echo "==> formula dump-diff ${#FILES[@]} files (optional smash2 cross-check vs smash2_cpp)"
else
  echo "==> formula dump ${#FILES[@]} files (C++-only; CROSS_ORACLE=1 for smash2 diff)"
fi
for rel in "${FILES[@]}"; do
  f="$ROOT/$rel"
  if [[ ! -f "$f" ]]; then
...
if [[ "$GATE_ORACLE" == "1" ]]; then
  echo "dump-formula corpus gate: PASS (byte-for-byte vs smash2; both sides exit 0)"
else
  echo "dump-formula corpus gate: PASS (C++ dumps well-formed; CROSS_ORACLE=1 for smash2 diff)"
fi
