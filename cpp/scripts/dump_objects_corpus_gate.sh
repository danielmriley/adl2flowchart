#!/usr/bin/env bash
# object_table dump gate over cpp/tests/objects_gate_files.txt.
...
if [[ "$GATE_ORACLE" == "1" ]]; then
  echo "==> objects dump-diff ${#FILES[@]} files (optional smash2 cross-check vs smash2_cpp)"
else
  echo "==> objects dump ${#FILES[@]} files (C++-only; CROSS_ORACLE=1 for smash2 diff)"
fi
for rel in "${FILES[@]}"; do
  f="$ROOT/$rel"
  if [[ ! -f "$f" ]]; then
...
if [[ "$GATE_ORACLE" == "1" ]]; then
  echo "dump-objects corpus gate: PASS (byte-for-byte vs smash2; both sides exit 0)"
else
  echo "dump-objects corpus gate: PASS (C++ dumps well-formed; CROSS_ORACLE=1 for smash2 diff)"
fi
