#!/usr/bin/env bash
# verify_corpus_gate.sh — smash2 verify corpus gate (trustworthy-verify M4).
#
# Runs `smash2 verify` over every `.adl` under examples/, aggregates pairwise
...
# Env:    SMASH2_BIN=path   skip the build and use this binary
#         VERIFY_CORPUS_OUT=dir   keep per-file .out/.err (default: mktemp)
#         VERIFY_FILE_TIMEOUT=secs   per-file timeout (default: 180)
#         SMASH2_VERIFY_ARGS=...     extra args (e.g. --no-refute-gate for a fast canary)
set -euo pipefail
...
if [[ -n "${SMASH2_BIN:-}" ]]; then
    smash2="$SMASH2_BIN"
    echo "verify_corpus_gate: using SMASH2_BIN=$smash2"
else
    echo "verify_corpus_gate: building smash2 (Workaround A: no native z3)..."
    cargo build --release -q --manifest-path "$workspace/Cargo.toml" \
        -p adl-cli --no-default-features
    smash2="$workspace/target/release/smash2"
fi
if [[ ! -x "$smash2" ]]; then
    echo "verify_corpus_gate: binary not executable: $smash2" >&2
    exit 1
fi
...
for f in "${files[@]}"; do
    ...
    if command -v timeout >/dev/null 2>&1; then
        timeout --signal=KILL "${timeout_secs}s" \
            "$smash2" verify "${extra_args[@]}" "$f" >"$OUT/$name.out" 2>"$OUT/$name.err"
        rc=$?
    else
        "$smash2" verify "${extra_args[@]}" "$f" >"$OUT/$name.out" 2>"$OUT/$name.err"
        rc=$?
