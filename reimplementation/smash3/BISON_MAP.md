...
Each nonterminal maps to one function in `crates/adl-syntax/src/parser.rs`.
...
| Bison idea | smash3 |
|---|---|
| Keyword tokens | `TokKind` in `crates/adl-syntax/src/token.rs`. Case-insensitive. |
| `IDENT` / numbers / strings | `Ident`, `Int` / `Real`, `String` |
| Path tokens | Not lexed. `try_path_token` in arg position only. |
