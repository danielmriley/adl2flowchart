//! `smash3-recheck` — standalone replay of `--combine` certificate bundles.
//!
//! Re-checks each bundle with the trusted exact-rational kernel only: no
//! solver, no search, no smash3 analysis run. A bundle that replays proves
...
//! them with a current smash3.
//!
//! Usage: `smash3-recheck BUNDLE.json... | DIR...`  (a directory contributes
...
//! release script that runs `smash3-recheck dist/bundles/` to gate a publish
            ...
            if found.is_empty() {
                ...
                return Err(format!(
                    "no *.json certificate bundles in directory {} ({}); \
                     ...
                     Bundles are produced by `smash3 verify --combine {}`",
                    p.display(),
                    match total {
...
/// Restore the default SIGPIPE disposition, so `smash3-recheck DIR/ | head`
...
fn main() -> ExitCode {
    ...
    if args.is_empty() || args.iter().any(|a| a == "--help" || a == "-h") {
        eprintln!("usage: smash3-recheck BUNDLE.json... | DIR...");
        eprintln!("re-checks smash3 `verify --combine` certificate bundles ({BUNDLE_SCHEMA})");
