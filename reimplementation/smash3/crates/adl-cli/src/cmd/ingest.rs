//! `smash3 ingest` — materialize a profile-mapped ROOT event file as
//! canonical JSONL, and/or emit the independent uproot oracle script
//! (SPEC_EVENT_PIPELINE §1.1).
//!
//! - `smash3 ingest events.root --profile delphes -o events.jsonl`
//!   reads natively (oxyroot) and writes byte-deterministic JSONL.
//! - `smash3 ingest --profile delphes --emit-script DIR` writes
