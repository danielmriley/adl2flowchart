//! `smash3 run` — evaluate an ADL file's regions over a JSONL event file
...
/// The §6 provenance `tool` string: `smash3 <crate version>`. No git hash
/// (no build-time capture wired) — deterministic per build, which the
/// determinism guarantee needs.
fn tool_id() -> String {
    format!("smash3 {}", env!("CARGO_PKG_VERSION"))
}
