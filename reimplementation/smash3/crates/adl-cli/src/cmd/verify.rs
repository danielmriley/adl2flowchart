//! `smash3 verify` — the legacy `smash -r` equivalent: full analysis with
...
fn write_bundles(
    ...
    eprintln!(
        "{unit}: wrote {} certificate bundle(s) to {} (re-check offline with smash3-recheck)",
...
fn expand_adl_inputs(inputs: &[PathBuf]) -> Result<Vec<PathBuf>, CliError> {
    ...
    let mut push_unique = |p: PathBuf, out: &mut Vec<PathBuf>| {
        ...
        if seen.insert(key) {
            ...
            eprintln!("smash3: ignoring duplicate input {}", p.display());
        }
    };
    for p in inputs {
        if p.is_dir() {
