//! `smash3` — the ADL2 command-line tool (SPEC_ARCHITECTURE §1/§9).
//!
//! Subcommands:
...
#[command(
    name = "smash3",
    version,
    about = "Run ADL analyses over events, then check, verify, visualize, ingest",
...
enum Command {
    ...
    Verify {
        ...
        /// `smash3-recheck` — no solver, no smash3 run. Byte-deterministic:
...
/// `smash3 verify --explain BIG.adl | head` used to die with
...
/// to a crashed solver killed smash3 outright — the broken-solver report
...
/// SIGPIPE kill produces, so `smash3 … | head` still looks like every other
...
mod tests {
    ...
    #[test]
    fn run_is_first_subcommand() {
        let cmd = super::Cli::command();
        let first = cmd.get_subcommands().next().expect("has subcommands");
        assert_eq!(first.get_name(), "run");
    }
}
