//! End-to-end CLI tests for `smash3`: snapshot the machine-clean stdout of
...
/// Run `smash3 run --histos DIR --csv --svg` on the ex02 golden, returning
...
fn run_histos_writes_native_root_file() {
    ...
    assert_eq!(prov["tool"], "smash3 0.1.0");
...
assert parsed["tool"] == "smash3 0.1.0", parsed["tool"]
