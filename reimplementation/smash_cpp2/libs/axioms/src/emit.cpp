...
struct Emit {
  ...
  void twin(const std::vector<QuantityId>& qs) {
    // smash3 emit uses twin_pairs (same oriented kind, reversed args,
    // a != b). Unindexed legs are still skipped: min-over-product is
    // not ± of the reverse pair (LANGUAGE.md / OPEN-2).
    std::set<QuantityId> set(qs.begin(), qs.end());
    for (auto [q1, q2] : twin_pairs(hir->table, set)) {
