...
port). Keep thin. Not rewritten for smash_cpp2 — U07 ports the
existing kernel so uncertified solver-UNSAT can stay CANDIDATE and
CLI verify can keep smash2_cpp certify-on. `--combine` and
`smash_cpp2-recheck` are later units. Schema stays `smash2-combine/2`.
