#!/usr/bin/env bash
# Stretch: compare smash_cpp2 check --dump-axioms vs smash3 on every
# examples/**/*.adl. Stdout only. Count ok/fail; do not invent axioms
# if a file drifts.
set -euo pipefail
...
echo "corpus dump-axioms: ok=$ok fail=$fail total=$((ok + fail))"
if [[ "$fail" -ne 0 ]]; then
  exit 1
fi
