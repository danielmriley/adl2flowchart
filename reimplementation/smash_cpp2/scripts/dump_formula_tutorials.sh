#!/usr/bin/env bash
# Compare smash_cpp2 check --dump-formula vs smash3 on
# examples/tutorials/*.adl. Stdout only. Gate for U05.
set -euo pipefail
...
echo "tutorials dump-formula: ok=$ok fail=$fail total=$((ok + fail))"
if [[ "$fail" -ne 0 ]]; then
  exit 1
fi
